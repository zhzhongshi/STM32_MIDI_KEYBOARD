...
/*
midiMessage(MIDI_NOTE_ON, Channel, Key , Vel);
midiMessage(MIDI_NOTE_OFF, Channel, Key , Vel);
midiMessage(MIDI_CONTROL_CHANGE, Channel, CC_Number , Value);
midiMessage(MIDI_PITCHBEND_CHANGE, Channel, byte1 , byte0);
...
void changeProgram(uint8_t message, uint8_t channel, uint8_t data1)
{
	if (_bufferIndex >= 63 - 4)
	{
		// Discard unsent messages
